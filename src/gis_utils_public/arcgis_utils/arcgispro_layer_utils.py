...
- set_cim_popup_info_fields_by_yml: Configure popup to inherit visible fields from layer (useLayerFields=True).
"""
...
def add_sde_layers_and_tables_to_map(
    map_obj: Any,
    layer_entries: list[tuple[str, dict[str, Any]]],
    sde_connection_by_layer_name: dict[str, str],
    default_sde_connection_path: str | None = None,
) -> dict[str, Any]:
    """Add pre-resolved layers/tables from SDE to the target map.

    :param map_obj: ArcGIS Pro map object.
    :param layer_entries: Ordered ``(layer_name, layer_config)`` entries.
    :param sde_connection_by_layer_name: Mapping ``layer_name -> sde_connection_path``.
    :param default_sde_connection_path: Optional fallback SDE path.
    :return: Summary dictionary with per-layer results.
    """
    if not isinstance(layer_entries, list):
        raise ValueError("layer_entries must be provided as a list")
    if not isinstance(sde_connection_by_layer_name, dict):
        raise ValueError("sde_connection_by_layer_name must be provided as a dict")
    results: list[dict[str, Any]] = []
    existing_layer_names = _get_existing_map_layer_names(map_obj)

    LOGGER.debug("Add layers/tables from SDE in provided order")
    ...
    for layer_name, layer_config in layer_entries:
        ...
        if normalized_layer_name in existing_layer_names:
            ...
            LOGGER.debug(
                "-> Skip adding layer '%s': layer already exists in map", layer_name
            )
            results.append(layer_result)
            continue

        LOGGER.debug("-> Add layer from SDE source: %s", layer_name)
        sde_connection_path = sde_connection_by_layer_name.get(layer_name)
        if not isinstance(sde_connection_path, str) or not sde_connection_path.strip():
            sde_connection_path = default_sde_connection_path
        ...
        if not isinstance(sde_connection_path, str) or not sde_connection_path.strip():
            ...
            layer_result["error"] = "No SDE connection available for layer"
            LOGGER.warning(
                "-> Could not resolve sde_connection for '%s': %s",
                layer_name,
                layer_result["error"],
            )
            results.append(layer_result)
        ...
        if not os.path.exists(sde_connection_path):
            LOGGER.warning(
                "-> SDE connection file was not found: %s", sde_connection_path
            )
        ...
        if layer_result.get("added"):
            ...
            LOGGER.debug(
                "-> Added layer '%s' from %s",
                layer_name,
                layer_result.get("dataset_path"),
            )
        elif layer_result.get("skipped"):
            LOGGER.debug("-> Skipped layer '%s'", layer_name)
        else:
            LOGGER.warning(
                "-> Failed adding layer '%s': %s", layer_name, layer_result.get("error")
            )
        results.append(layer_result)
    ...
    try:
        ...
        if mode == "all":
            if hasattr(source_layer, "definitionQuery") and hasattr(
                ...
                target_layer.definitionQuery = (
                    source_query if source_query is not None else ""
                )
            ...
            if hasattr(source_layer, "showLabels") and hasattr(
                target_layer, "showLabels"
            ):
...
def apply_lyrx_to_map_layers(
    ...
    """Apply LYRX transfer for an explicit list of map layers.

    :param map_obj: ArcGIS Pro map object.
    :param layer_names: Ordered list of target map layer names.
    :param lyrx_dir: Root LYRX directory.
    :param transfer_mode: Transfer mode (``symbology`` or ``all``).
    :param allow_suffix_match: Whether to allow ``<layer_name>_*.lyrx`` fallback.
    :param recurse_subfolders: Whether LYRX lookup recurses into subfolders.
    ...
    if not isinstance(layer_names, list):
        raise ValueError("layer_names must be provided as a list")
    target_layer_names = [
        name.strip() for name in layer_names if isinstance(name, str) and name.strip()
    ]
    results: list[dict[str, Any]] = []
    ...
    for layer_name in target_layer_names:
        ...
        if not target_layers:
            ...
            LOGGER.warning(
                "-> Could not find target map layer for LYRX transfer: %s", layer_name
            )
    ...
    return {
        "layer_results": results,
        "layer_total": len(target_layer_names),
...
def export_map_layers_to_lyrx(
    ...
    def _export_one(layer_name: str, target_layer: Any) -> None:
        ...
        if prefix:
            layer_stem = (
                re.sub(r"[^0-9a-z_]+", "_", layer_name.lower()).strip("_") or "layer"
            )
            ...
            lyrx_filename = (
                f"{file_stem}_{suffix}.lyrx" if suffix else f"{file_stem}.lyrx"
            )
        ...
        try:
            ...
            _append_result(
                layer_name, lyrx_path, False, False, str(exc), "export_failed"
            )
...
def get_field_names_from_yml(layer: Any, configured_cols: Any) -> tuple[list[str], str]:
    ...
    if isinstance(configured_cols, list):
        resolved = [
            field_name
            for field_name in configured_cols
...
def order_cim_feature_table_field_descriptions_by_yml(
    layer: Any, desired_field_order: list[str]
) -> int | None:
    ...
    if not desired_field_order:
        LOGGER.debug(
            "order_cim_feature_table_field_descriptions_by_yml: skip '%s' (no cols configured)",
            layer_name,
        )
        return 0
    ...
    if not field_descriptions:
        LOGGER.debug(
            "order_cim_feature_table_field_descriptions_by_yml: skip '%s' (no field descriptions)",
            layer_name,
        )
        return None
    ...
    if reordered_descriptions == field_descriptions:
        LOGGER.debug(
            "order_cim_feature_table_field_descriptions_by_yml: already ordered for '%s'",
            layer_name,
        )
        return 0
...
def set_cim_feature_table_field_descriptions_from_sde(layer: Any) -> tuple[int, int]:
    ...
    if fields is None or cim is None:
        LOGGER.debug(
            "set_cim_feature_table_field_descriptions_from_sde: skip '%s' (no fields available)",
            layer_name,
        )
        return 0, 0
    ...
    if feature_table is None:
        LOGGER.debug(
            "set_cim_feature_table_field_descriptions_from_sde: skip '%s' (no featureTable on CIM)",
            layer_name,
        )
        return 0, 0
    ...
    for field in fields:
        ...
        field_alias = (
            field.get("alias")
            if isinstance(field, dict)
            else getattr(field, "aliasName", None)
        )
    ...
    if not rebuilt:
        LOGGER.debug(
            "set_cim_feature_table_field_descriptions_from_sde: skip '%s' (could not build field descriptions)",
            layer_name,
        )
        return 0, 0
...
def set_cim_feature_table_field_descriptions_visibility_by_yml(
    ...
    if not visible_field_names:
        LOGGER.debug(
            "set_cim_feature_table_field_descriptions_visibility_by_yml: skip '%s' (no cols configured)",
            layer_name,
        )
        return 0, 0, 0, 0, 0, []
    ...
    if field_descriptions is None:
        LOGGER.debug(
            "set_cim_feature_table_field_descriptions_visibility_by_yml: skip '%s' (no field descriptions)",
            layer_name,
        )
        return 0, 0, 0, 0, 0, []
...
def set_cim_layer_definition_visibility(
    ...
    if expected_visible is None:
        LOGGER.debug(
            "set_cim_layer_definition_visibility: skip '%s' (no visible configured)",
            item_name,
        )
    ...
    if not isinstance(expected_visible, bool):
        LOGGER.debug(
            "set_cim_layer_definition_visibility: skip '%s' (invalid visible value)",
            item_name,
        )
    ...
    try:
        ...
        if current_visible == expected_visible:
            LOGGER.debug(
                "set_cim_layer_definition_visibility: '%s' visibility=%s (no change needed)",
                item_name,
                current_visible,
            )
        ...
        LOGGER.debug(
            "set_cim_layer_definition_visibility: '%s' visibility=%s (updated)",
            item_name,
            expected_visible,
        )
        ...
        LOGGER.warning(
            "set_cim_layer_definition_visibility: failed for '%s': %s", item_name, exc
        )
...
def check_and_update_definition_query(
    ...
    if expected_query is None:
        LOGGER.debug(
            "check_and_update_definition_query: skip '%s' (no definition_query configured)",
            item_name,
        )
    ...
    if not isinstance(expected_query, str):
        LOGGER.debug(
            "check_and_update_definition_query: skip '%s' (invalid definition_query)",
            item_name,
        )
    ...
    if hasattr(item, "definitionQuery"):
        try:
            ...
            current_query = (
                current_query_raw.strip() if isinstance(current_query_raw, str) else ""
            )
    ...
    try:
        ...
        LOGGER.warning(
            "check_and_update_definition_query: failed for '%s': %s", item_name, exc
        )
        return None, False, f"Could not set definition query: {exc}"


def set_cim_layer_definition_service_id(
    layer: Any, expected_service_id: Any
) -> tuple[bool, bool]:
    ...
    if expected_service_id is None:
        LOGGER.debug(
            "set_cim_layer_definition_service_id: skip '%s' (no service_id configured)",
            layer_name,
        )
        return True, False
    ...
    try:
        ...
        if current_id == expected_id:
            LOGGER.debug(
                "set_cim_layer_definition_service_id: already correct for '%s' (%s)",
                layer_name,
                current_id,
            )
            return True, False
        ...
        LOGGER.warning(
            "set_cim_layer_definition_service_id: failed for '%s': %s", layer_name, exc
        )
        return False, False
...
def set_cim_feature_table_display_field(
    ...
    if not isinstance(display_field_name, str) or not display_field_name.strip():
        LOGGER.debug(
            "set_cim_feature_table_display_field: skip '%s' (no display_field configured)",
            layer_name,
        )
    ...
    if requested_normalized is None:
        LOGGER.debug(
            "set_cim_feature_table_display_field: skip '%s' (empty display_field)",
            layer_name,
        )
    ...
    if error_message:
        LOGGER.warning(
            "set_cim_feature_table_display_field: field lookup failed for '%s': %s",
            layer_name,
            error_message,
        )
    ...
    if requested_normalized not in available_by_normalized:
        ...
        return (
            None,
            False,
            f"Configured display_field '{requested}' not found in layer fields",
        )
    ...
    try:
        ...
        LOGGER.warning(
            "set_cim_feature_table_display_field: failed for '%s': %s", layer_name, exc
        )
...
def set_cim_popup_info_fields_by_yml(
    ...
    if cim_module is None:
        LOGGER.warning(
            "set_cim_popup_info_fields_by_yml: arcpy.cim missing for '%s'", layer_name
        )
    ...
    try:
        ...
        configured_popup_description_template = (
            ...
            if isinstance(popup_description_template, str)
            and popup_description_template.strip()
            else None
        )
        ...
        display_field_name = (
            getattr(feature_table, "displayField", None)
            if feature_table is not None
            else None
        )
        display_field_token = ""
            ...
            if isinstance(field_descs, list):
                for field_desc in field_descs:
                    ...
                    if (
                        ...
                        and normalize_field_name(field_name)
                        == normalize_field_name(display_field_name)
                    ):
                        alias = getattr(field_desc, "alias", None) or getattr(
                            field_desc, "fieldAlias", None
                        )
        ...
        if configured_popup_description_template is not None:
            ...
            caption_text = caption_text.replace(
                "{display_field_value}", display_field_token
            )
        ...
        if current_popup is not None:
            current_media = getattr(current_popup, "mediaInfos", None)
            if isinstance(current_media, list) and current_media:
                current_use_layer_fields = getattr(
                    current_media[0], "useLayerFields", None
                )
                ...
                if (
                    current_use_layer_fields is True
                    and current_title == expected_title
                    and current_fields == visible_field_names
                    and current_caption == caption_html
                ):
                    LOGGER.debug(
                    ...
                    return (
                        True,
                        False,
                        f"Popup configured with {len(visible_field_names)} visible fields",
                    )
        ...
        if (
            ...
            popup_info = current_popup or popup_ns.CIMPopupInfo()
        else:
            table_media = cim_module.CreateCIMObjectFromClassName(
                "CIMTableMediaInfo", "V3"
            )
            popup_info = current_popup or cim_module.CreateCIMObjectFromClassName(
                "CIMPopupInfo", "V3"
            )
        ...
        return (
            False,
            True,
            f"Popup configured with {len(visible_field_names)} visible fields, title {expected_title!r}",
        )
    except Exception as exc:
        LOGGER.warning(
            "set_cim_popup_info_fields_by_yml: failed for '%s': %s", layer_name, exc
        )
